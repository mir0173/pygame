    ...
    def move(self, screen, dt, game_speed):
        ...
        for i in range(len(self.obstacle)):
            if self.left_right == 1:
                if self.player.top + bird_top_area < self.obstacle[i].top < self.player.bottom - bird_bottom_area and ifcrash(
        ...
        for i in range(len(self.obstacle)):
            if self.left_right == 1:
                if self.player.top + bird_top_area < self.obstacle[i].bottom < self.player.bottom - bird_bottom_area and ifcrash(
                        self.player.left + bird_left_area, self.player.right - bird_right_area, self.obstacle[i].left,
                        self.obstacle[i].right):
                    b = 0
            if self.left_right == 0:
