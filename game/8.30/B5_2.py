...
from interaction import interaction, diaryinter
...
wadpos = pygame.Rect(700, 700, 200, 100)
#wadpos 는 일지 위치 찍어주는거
        ...
        while run:
            ...
            if not self.pause:
                ...
                if self.screen_number == 10:
                    ...
                    if k == 1:
                        k = diaryinter(protagonist.player, wadpos, self.screen, self.font, 3, 3, protagonist)
                        # 숫자 2개만 바꾸면 되는데 앞에거는 시작 인덱스고 뒤에거는 이번 일지 개수임 여기서는 4번째(배열의 3은 4번째니까)거부터 3개를 튼다는 뜻
                    if k == 5:
                        print(k)
                        canmove = True
