...
class protagonist:
    def __init__(self, font, image1, image2, i):
        ...
        self.ui7 = pygame.image.load('./assets/ui7.png')
        self.ui7 = pygame.transform.scale(self.ui7, (1600, 900))
        self.ui7.set_colorkey((255, 255, 254))
        self.uilist = [self.ui, self.ui2, self.ui3, self.ui4, self.ui5, self.ui6, self.ui7]
    ...
    def move(self, screen, dt, game_speed, npc_list):
        ...
        update(screen, self.font, self, npc_list)
        ...
        if self.screen_number == 19:
            if self.player.left <= 400:
                text1 = pygame.transform.scale(pygame.image.load('./assets/tutorial5.png'), (700, 41))
                text1.set_colorkey((0, 0, 0))
                screen.blit(text1, (650, 300))
                text2 = pygame.transform.scale(pygame.image.load('./assets/tutorial6.png'), (700, 45))
                text2.set_colorkey((0, 0, 0))
                screen.blit(text2, (650, 400))
            text3 = pygame.image.load('./assets/tutorial7.png')
            text3.set_colorkey((0, 0, 0))
            screen.blit(text3, (50, 200))
        if self.screen_number == 43:
            text1 = pygame.image.load('./assets/tutorial9.png')
            text1.set_colorkey((0, 0, 0))
            screen.blit(text1, (650, 300))
            text2 = pygame.image.load('./assets/tutorial10.png')
            text2.set_colorkey((0, 0, 0))
            screen.blit(text2, (650, 400))
    ...
    def force_move(self, screen, dt, game_speed, destination, last_lr) -> int:
        self.frame_1.set_alpha(self.alpha)
        self.frame_2.set_alpha(self.alpha)
        self.frame_3.set_alpha(self.alpha)
        self.frame_4.set_alpha(self.alpha)
        ...
        if last_lr == 0:
            ...
            self.left_right = 1
        if destination <= self.player.left:
            self.left_right = 0
            self.moving = True
            self.player.left -= game_speed * dt
            self.sound = (self.sound + 1) % 6
            if current_time - self.last_update >= dt * 4:
                self.frame = (self.frame + 1) % 4
                self.last_update = current_time
                screen.blit(self.animation_list2[self.frame], self.player)
                ...
                if self.sound == 0:
                    self.walking[randint(0, 4)].play()
        elif destination >= self.player.right:
            self.left_right = 1
            self.moving = True
            self.player.left += game_speed * dt
            self.sound = (self.sound + 1) % 6
...
class NPC(pygame.sprite.Sprite):
    def __init__(self, font, image1, image2, lr, initial_position):
        ...
        self.frame = 0
        self.run = True
        self.npc = pygame.Rect(self.initial_position, 775 - 320 * 0.5 + bottom_area, 320 * 0.5, 320 * 0.5)
        self.left_right = lr
    ...
    def force_move(self, screen, dt, game_speed, destination, last_lr) -> int:
        ...
        if last_lr == 0:
            ...
            self.left_right = 1
        if destination <= self.npc.left:
            self.left_right = 0
            self.moving = True
            self.npc.left -= game_speed * dt
            self.sound = (self.sound + 1) % 4
            if current_time - self.last_update >= dt * 4:
                self.frame = (self.frame + 1) % 4
                self.last_update = current_time
                ...
                if self.sound == 0:
                    self.walking[randint(0, 4)].play()
        elif destination >= self.npc.right:
            self.left_right = 1
            self.moving = True
            self.npc.left += game_speed * dt
            self.sound = (self.sound + 1) % 4
            if current_time - self.last_update >= dt * 4:
                self.frame = (self.frame + 1) % 4
                self.last_update = current_time
                self.npc_image = self.animation_list2[self.frame]
                if self.sound == 0:
                    self.walking[randint(0, 4)].play()
        if not self.moving:
            return 1
    ...
    def npc_update(self, screen):
        ...
        if self.visible:
            if self.moving:
                if self.left_right == 0:
                    screen.blit(self.animation_list2[self.frame], self.npc)
                else:
