...
def playtext(screen, font, protagonist, npc_list):
    ...
    if text[textenum][index] == 0 and index > 0:
        endtext(screen, font, textenum, protagonist, npc_list)
        return 1

    textPlayer = font[0].render(text[textenum][index], True, WHITE)
    update(screen, font, protagonist, npc_list)
    if textenum == 59:
        michael_final = pygame.transform.scale(pygame.image.load('./assets/michael_final (1).png'),
                                               (540, 540))
        michael_final.set_colorkey((0, 0, 0))
        screen.blit(michael_final, (900, 265))
...
def starttext(screen, k, font, protagonist, npc_list):
    global index, textenum
    if (index != -1):
        return
    index += 1
    ...
    for n in range(1, 50):
        ...
        pygame.time.wait(5)
    textenum = k
    playtext(screen, font, protagonist, npc_list)


def endtext(screen, font, k, protagonist, npc_list):
    global index
    for n in range(1, 50):
        update(screen, font, protagonist, npc_list)
        if textenum == 59:
