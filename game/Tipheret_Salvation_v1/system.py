...
index = [[6, 1, 0], [6, 2, 0], [6, 2, 0], [6, 2, 0], [6, 2, 0], [6, 3, 0], [6, 3, 0], [5, 1, 1], [5, 1, 1], [5, 2, 1],
         [5, 2, 1], [5, 2, 1], [5, 2, 1], [5, 3, 1], [5, 3, 1], [5, 3, 1], [5, 3, 1], [5, 4, 1], [5, 4, 1], [4, 1, 2],
         [4, 2, 2], [4, 3, 2], [4, 3, 2], [4, 3, 2], [4, 4, 2], [4, 5, 2], [3, 1, 3], [3, 2, 3], [0, 1, 6], [0, 2, 6],
         [0, 3, 6], [0, 4, 6], [0, 4, 6], [3, 2, 3], [3, 3, 3], [3, 3, 3], [2, 1, 4], [2, 1, 4], [2, 1, 4],
         [2, 2, 4], [2, 2, 4], [0, 5, 6], [0, 5, 6], [0, 5, 6], [2, 2, 4], [2, 2, 4], [2, 2, 4], [2, 3, 4], [1, 1, 5], [1, 2, 5], [1, 3, 5], [0, 6, 6], [0, 6, 6], [1, 3, 5], [0, 6, 6], [0, 6, 6], [1, 3, 5]]

BLACK = pygame.Color(0, 0, 0)
WHITE = pygame.Color(255, 255, 254)
...
pausepopup_data = pygame.transform.scale(pygame.image.load('./assets/pause_data.png'), (1600, 900))
pausepopup_data.set_colorkey(WHITE)
...
pauseresume_data = pygame.transform.scale(pygame.image.load('./assets/pause_resume_data.png'), (1600, 900))
pauseresume_data.set_colorkey(WHITE)
...
pauseexit_data = pygame.transform.scale(pygame.image.load('./assets/pause_exit_data.png'), (1600, 900))
pauseexit_data.set_colorkey(WHITE)
...
pausemain_data = pygame.transform.scale(pygame.image.load('./assets/pause_main_data.png'), (1600, 900))
pausemain_data.set_colorkey(WHITE)
...
pausesave_data = pygame.image.load('./assets/pause_save_data.png')
pausesave_data = pygame.transform.scale(pausesave_data, (1600, 900))
pausesave_data.set_colorkey(WHITE)
...
def ui(floor, ori, data, ):
    if index[floor.screen_number][0] == 0:
        return data
    return ori


def esc(floor, screen, font, protagonist, npc_list):
    ...
    for i in range(6):
        ...
        screen.blit(ui(floor, pausepopup, pausepopup_data), (0, 900 - 150 * i))
        screen.blit(ui(floor, pausemain, pausemain_data), (96 - 16 * i, -5))
...
def save(screen, font, protagonist, number):
    ...
    while X:
        for event in pygame.event.get():
            ...
            if not mouse_clicked and 282 <= pos[0] <= 347 and 641 <= pos[1] <= 705 and click[0] == 1 and saveindex > 0:
                saveindex -= 1
                mouse_clicked = True
            if not mouse_clicked and 352 <= pos[0] <= 421 and 641 <= pos[1] <= 705 and click[0] == 1 and saveindex < 4:
                saveindex += 1
                mouse_clicked = True
            if not click[0]:
                mouse_clicked = False
            if not mouse_clicked and 1144 <= pos[0] <= 1351 and 641 <= pos[1] <= 705 and click[0] == 1 and number == 1:
...
def resume(floor, screen, font, protagonist, npc_list):
    global key
    update(screen, font, protagonist, npc_list)
    pos = pygame.mouse.get_pos()
    if 647 <= pos[0] <= 951 and 290 <= pos[1] <= 364:
        screen.blit(ui(floor, pauseresume, pauseresume_data), (0, 0))
    ...
    if 647 <= pos[0] <= 951 and 290 <= pos[1] <= 363 and click[0] == 1:
        for i in range(6):
            ...
            dt = clock.tick(60)
            update(screen, font, protagonist, npc_list)
            screen.blit(ui(floor, pausepopup, pausepopup_data), (0, 0 + 150 * i))
            screen.blit(ui(floor, pausemain, pausemain_data), (0 + 16 * i, -5))
            pygame.display.update()
        floor.pause = False
...
def update(screen, font, protagonist, npc_list):
